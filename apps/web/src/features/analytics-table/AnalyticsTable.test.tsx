...
describe('AnalyticsTablePanel', () => {
  it('показывает пять столбцов и агрегаты в русском формате', () => {
    renderWithProviders(<AnalyticsTablePanel model={model} selectedId={null} onSelect={noop} />);
  ...
  it('клик по строке вызывает onSelect с id узла', async () => {
    ...
    renderWithProviders(
      <AnalyticsTablePanel model={model} selectedId={null} onSelect={onSelect} />,
    );
  ...
  it('выделенная строка помечена aria-selected', () => {
    renderWithProviders(<AnalyticsTablePanel model={model} selectedId="dep-b1" onSelect={noop} />);
